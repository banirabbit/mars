...
class APIRecommendationService:
    ...
    def recommend_apis(self, question: Dict[str, Any],
        ...
        Args:
            ...
            top_k (Optional[int]): Number of APIs to recommend (default from config)

        Returns:
            Dict[str, Any]: Multi-agent recommendation result with call logs
        """
        ...
        if top_k is None:
            top_k = getattr(self.config.evaluation, 'llm_predict_limit', 10)

        # Create mashup text for processing
        mashup_text = create_mashup_text(question)

        # Run multi-agent pipeline with top_k
        try:
            ...
            if agent_result is None:
                ...
                return {"related_apis": [], "call_logs": [], "total_time": 0.0, "total_tokens": 0, "iterations": 0}
            return agent_result
        except Exception as e:
    ...
    def recommend_with_retry(self, question: Dict[str, Any],
                           ...
                           hallucination_threshold: Optional[int] = None,
                           top_k: Optional[int] = None) -> tuple[List[str], int]:
        """
        ...
        Args:
            ...
            top_k (Optional[int]): Number of APIs to recommend
        ...
        while attempt_count < max_attempts:
            ...
            # Generate recommendations with top_k
            response = self.recommend_apis(question, candidate_apis, top_k=top_k)
            recommendations = self.process_recommendation_response(response, limit=top_k)
